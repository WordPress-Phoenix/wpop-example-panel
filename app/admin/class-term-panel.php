...
class Term_Panel {
	...
	function __construct( $installed_dir, $installed_url ) {
		...
		$this->setup_term_options();
	}

	/**
	 * Register Options Panel
	 */
	function setup_term_options() {
		...
		$this->term_meta = new Opts\page(
			$this->term_meta_config(),
			$sections
		);

		// initialize_panel() is a function in the opt panel class
		$this->term_meta->initialize_panel();
	}

	function term_meta_config() {
		return array(
			'parent_page_id' => 'edit.php',
			'id'             => 'wpop-term-panel',
			'page_title'     => 'WPOP Term Panel Settings',
			'menu_title'     => 'WPOP Term Panel',
