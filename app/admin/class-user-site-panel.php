...
class User_Site_Panel {
	 ...
	 * @var string
	 */
	protected $user_site_meta;

	/**
	 * User_Site_Panel constructor.
	 *
	...
	function __construct( $installed_dir, $installed_url ) {
		...
		$this->setup_user_site_panel();
	}

	/**
	 * Register Options Panel
	 */
	function setup_user_site_panel() {
		...
		$this->user_site_meta = new Opts\page(
			$this->site_options_config(),
			$sections
		);

		// initialize_panel() is a function in the opt panel Container class
		$this->user_site_meta->initialize_panel();
	}

	function site_options_config() {
		return array(
			'parent_page_id' => 'users.php',
			'id'             => 'wpop-example-users-site-opts',
			'page_title'     => 'WPOP ' . esc_attr( get_bloginfo( 'name' ) ) .' Options',
			'menu_title'     => 'WPOP User Site Panel',
