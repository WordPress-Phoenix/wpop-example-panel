...
class User_Network_Panel {
	 ...
	 * @var string
	 */
	protected $user_network_option;

	/**
	 * User_Network_Panel constructor.
	 *
	...
	function __construct( $installed_dir, $installed_url ) {
		...
		$this->setup_user_network_panel();
	}

	/**
	 * Register Options Panel
	 */
	function setup_user_network_panel() {
		...
		$this->user_network_option = new Opts\page(
			$this->site_options_config(),
			$sections
		);

		// initialize_panel() is a function in the opt panel Container class
		$this->user_network_option->initialize_panel();
	}

	function site_options_config() {
		return array(
			'parent_page_id' => 'users.php',
			'id'             => 'wpop-example-network-user',
			'page_title'     => 'WPOP User Panel',
			'menu_title'     => 'WPOP User Panel',
